                                    ...
                                    struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    if (ctxt->op == BLE_GATT_ACCESS_OP_WRITE_CHR)
    {
        ...
        ESP_LOGI(TAG, "Received BLE provisioning raw data (len=%d): %s", len, (char *)rx_buf);
        /* 
         * 【预留配网接口点】：
...
static int ble_svc_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    switch (event->type)
    {
        case BLE_GAP_EVENT_CONNECT:
            ESP_LOGI(TAG, "BLE connection established; status=%d ", event->connect.status);
...
static void ble_svc_advertise(void)
{
    ...
    rc = ble_gap_adv_start(s_own_addr_type, NULL, BLE_HS_FOREVER,
                           &adv_params, ble_svc_gap_event, NULL);
    if (rc != 0)
    {
    ...
    ESP_LOGI(TAG, "BLE advertising successfully started as '%s'", BLE_DEVICE_NAME);
}
...
void ble_service_init(void)
{
    ...
    /* 3. 初始化内置 GAP 与 GATT 服务，并注册我们的自定义配网服务数据库 */
    ble_svc_gap_init();
    ble_svc_gatt_init();
...
esp_err_t ble_service_start(void)
{
    ...
    {
        BaseType_t ret = xTaskCreatePinnedToCore(ble_host_task, "ble_host", 4096, NULL, 5, &s_ble_host_task_handle, 0);
        if (ret != pdPASS)
        {
