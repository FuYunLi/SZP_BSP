...
esp_err_t bsp_audio_i2s_init(void)
{
    ...
    // 1. 分配 I2S0 控制器的 TX 通道（Master模式），并精简 DMA 参数以节约内部 SRAM
    i2s_chan_config_t chan_cfg = {
        .id = I2S_NUM_0,
        .role = I2S_ROLE_MASTER,
        .dma_desc_num = 4,      // 从默认的 6 降到 4，节约描述符内存
        .dma_frame_num = 128,   // 从默认的 240 降到 128，极大节约数据缓冲区 SRAM 占用
        .auto_clear = true,
    };
    esp_err_t ret = i2s_new_channel(&chan_cfg, &s_tx_handle, NULL);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "分配 I2S TX 通道失败: %s", esp_err_to_name(ret));
        return ret;
    }

    // 2. 配置 I2S0 STD 模式参数 (MCK->IO38, BCK->IO14, WS->IO13, DO->IO45)
